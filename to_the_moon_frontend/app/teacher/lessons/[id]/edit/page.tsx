...
export default function EditLessonPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      setSelectedStudentId(lessonData.student.id)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const result = await api.put(`/lessons/${params.id}`, updateData)
      console.log('Update successful:', result)
      toast({
        title: '성공',
        description: '수업 일정이 수정되었습니다',
      })
      // 짧은 딜레이 후 이동 (toast가 보이도록)
      setTimeout(() => {
      ...
      toast({
        title: '오류',
        description: error instanceof Error ? error.message : '수업 일정 수정에 실패했습니다',
        variant: 'destructive',
      })
    } finally {
      setIsLoading(false)
